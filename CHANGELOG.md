...
## [Unreleased]
### Added
- Internal shared infrastructure for upcoming `infra install --template`/`server install --template` support (built-in "workspace" templates, template resolution, post-start-action dispatch, and validation rejecting an unrecognized top-level or `post_start_actions` key instead of silently discarding it) — no user-facing change yet.
