...
### Fixed
- `.env` values containing a backslash, double quote, or newline (e.g. `DF_PLUGINS_SETUP` JSON) are no longer silently corrupted when a config or secrets file is re-read and re-written during install - such values are now quoted and escaped symmetrically on write and decoded correctly on read. A line that can't be parsed as `KEY=VALUE` now logs a warning instead of being dropped without any indication.
