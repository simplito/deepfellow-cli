...
- `deepfellow suite install --resume` self-healing a deleted infra directory no longer leaves the server pointed at a dead `DF_INFRA_API_KEY`. Reprovisioning infra from scratch mints a brand-new key, but `server install`'s own step only checked that the *server's* directory was still intact, with no awareness that infra had just rotated the key underneath it - the server step (config re-apply and container restart, since a running container never picks up an `.env` change on its own) is now also forced through whenever infra needed repair, not just when the server's own directory is missing.
