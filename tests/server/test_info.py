...
from deepfellow.server.info import ENV_METADATA, _dynamic_config_values, app, info
...
@mock.patch("deepfellow.server.info._dynamic_config_values", return_value={})
@mock.patch("deepfellow.server.info.print_env_info")
def test_info_cli_accepts_url_option(mock_print: Mock, mock_dynamic: Mock):
    result = runner.invoke(app, ["--url", SERVER])

    assert result.exit_code == 0
    assert mock_dynamic.call_args == mock.call(SERVER, False)
